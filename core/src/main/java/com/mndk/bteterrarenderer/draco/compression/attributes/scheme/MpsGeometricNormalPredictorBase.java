...
public abstract class MpsGeometricNormalPredictorBase<DataT> {
    ...
    protected MpsGeometricNormalPredictorBase(DataNumberType<DataT, ?> dataType, MeshPredictionSchemeData<?> meshData) {
    ...
    protected VectorD.L3 getPositionForDataId(int dataId) {
        ...
        positionAttribute.convertValue(posValId, pos);
        return pos;
    }
    ...
    protected VectorD.I2 getOctahedralCoordForDataId(int dataId, CppVector<DataT> data) {
        if(!this.isInitialized()) {
            throw new IllegalStateException("Not initialized");
        }
    ...
    protected abstract void computePredictedValue(CornerIndex cornerId, CppVector<DataT> prediction);
}
