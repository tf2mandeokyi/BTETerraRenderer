...
public class PSchemeNormalOctahedronCanonicalizedTransformBase<DataT>
    ...
    protected int getRotationCount(Point2 pred) {
    ...
    protected Point2 rotatePoint(Point2 p, int rotationCount) {
        DataNumberType<DataT> dataType = this.getDataType();
        switch (rotationCount) {
            case 1:
                return new Point2(p.get(1), dataType.negate(p.get(0)));
            case 2:
                return new Point2(dataType.negate(p.get(0)), dataType.negate(p.get(1)));
            case 3:
                return new Point2(dataType.negate(p.get(1)), p.get(0));
            default:
                return p;
        }
    }

    protected boolean isInBottomLeft(Point2 p) {
        DataNumberType<DataT> dataType = this.getDataType();
