...
@UtilityClass
public class PredictionSchemeFactory {

    public <DataT, CorrT> PredictionSchemeDecoder<DataT, CorrT> createMeshPredictionScheme(
            MeshDecoder source, PredictionSchemeMethod method,
            int attId, PredictionSchemeDecodingTransform<DataT, CorrT> transform, UShort bitstreamVersion)
    {
        PointAttribute att = source.getPointCloud().getAttribute(attId);
        if(source.getGeometryType() == EncodedGeometryType.TRIANGULAR_MESH && (
                method == PredictionSchemeMethod.MESH_PREDICTION_PARALLELOGRAM ||
                method == PredictionSchemeMethod.MESH_PREDICTION_MULTI_PARALLELOGRAM ||
                method == PredictionSchemeMethod.MESH_PREDICTION_CONSTRAINED_MULTI_PARALLELOGRAM ||
                method == PredictionSchemeMethod.MESH_PREDICTION_TEX_COORDS_PORTABLE ||
                method == PredictionSchemeMethod.MESH_PREDICTION_GEOMETRIC_NORMAL ||
                method == PredictionSchemeMethod.MESH_PREDICTION_TEX_COORDS_DEPRECATED)) {
            ...
            MeshPredictionSchemeDecoderFactory<DataT> factory = new MeshPredictionSchemeDecoderFactory<>();
            if(attCt != null) {
                MeshPredictionSchemeData<MeshAttributeCornerTable> md = new MeshPredictionSchemeData<>();
                ...
                MeshPredictionSchemeData<CornerTable> md = new MeshPredictionSchemeData<>();
