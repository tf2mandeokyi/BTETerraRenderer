...
public class MeshPredictionSchemeDecoderFactory<DataT> {

    public <CorrT> PredictionSchemeDecoder<DataT, CorrT> create(
            PredictionSchemeMethod method, PointAttribute attribute,
            PredictionSchemeDecodingTransform<DataT, CorrT> transform, MeshPredictionSchemeData<?> meshData,
            UShort bitstreamVersion)
    {
        switch (method) {
            case MESH_PREDICTION_PARALLELOGRAM:
                return new MpsParallelogramDecoder<>(attribute, transform, meshData);
            case MESH_PREDICTION_MULTI_PARALLELOGRAM:
                return new MpsMultiParallelogramDecoder<>(attribute, transform, meshData);
            case MESH_PREDICTION_CONSTRAINED_MULTI_PARALLELOGRAM:
                return new MpsConstrainedMultiParallelogramDecoder<>(attribute, transform, meshData);
            case MESH_PREDICTION_TEX_COORDS_DEPRECATED:
                return new MpsTexCoordsDecoder<>(attribute, transform, meshData, bitstreamVersion);
            case MESH_PREDICTION_TEX_COORDS_PORTABLE:
                return new MpsTexCoordsPortableDecoder<>(attribute, transform, meshData);
            case MESH_PREDICTION_GEOMETRIC_NORMAL:
                return new MpsGeometricNormalDecoder<>(attribute, transform, meshData);
            default:
                return null;
        }
    }
