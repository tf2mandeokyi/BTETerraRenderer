...
public abstract class GuiAbstractWidgetCopy extends GuiComponentCopy {
    ...
    public void drawComponent(Object poseStack) {
        ...
        RawGuiManager.drawButton(poseStack, x, y, width, height, hoverState);
        ...
        int stringWidth = FontManager.getStringWidth(buttonText);
        int ellipsisWidth = FontManager.getStringWidth("...");

        if (stringWidth > width - 6 && stringWidth > ellipsisWidth) {
            buttonText = FontManager.trimStringToWidth(buttonText, width - 6 - ellipsisWidth).trim() + "...";
        }
        FontManager.drawCenteredStringWithShadow(poseStack, buttonText, this.x + this.width / 2f, this.y + (this.height - 8) / 2f, color);
    }
