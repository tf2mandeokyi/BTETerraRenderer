...
public class NativeGuiScreenWrapperImpl extends MinecraftObjectWrapper<Screen> implements NativeGuiScreenWrapper {
    ...
    public void initGui(int width, int height) {
        getWrapped().init(MinecraftClient.getInstance(), width, height);
    }
    public void setScreenSize(int width, int height) {
        getWrapped().resize(MinecraftClient.getInstance(), width, height);
    }
    public void tick() {
        getWrapped().tick();
    }
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double scrollAmount) {
        return getWrapped().mouseScrolled(mouseX, mouseY, 0, scrollAmount);
    }
